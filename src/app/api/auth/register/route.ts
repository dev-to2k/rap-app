...
export async function POST(req: NextRequest) {
  ...
  const role = parsed.data.role as Exclude<UserRole, "admin">;

  const ipOk = await durableRateLimit(`register:ip:${ip}`, 10, 60_000);
  const emailOk = await durableRateLimit(`register:email:${email}`, 3, 60_000);
  if (!ipOk || !emailOk) {
