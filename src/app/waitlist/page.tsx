...
function WaitlistForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!res.ok) {
      ...
      setError(data.error || "Khong luu duoc");
      return;
    }
    setDone(true);
  }
  ...
  if (done) {
    return (
      <div className="mx-auto max-w-md space-y-4 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-6 text-center">
        <h1 className="text-xl font-bold text-white">Da nhan early access</h1>
        <p className="text-sm text-zinc-300">Cam on ban — minh se lien he qua email khi mo them slot.</p>
        <Link href="/" className="inline-block text-sm text-emerald-400 hover:underline">Ve cho beat</Link>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-md space-y-6">
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <fieldset className="space-y-2 text-sm">
          <legend className="text-zinc-400">Ban la</legend>
        ...
        <button type="submit" disabled={loading}
          ...
          {loading ? "Dang gui..." : "Xin early access"}
        </button>
      </form>
    </div>
  );
