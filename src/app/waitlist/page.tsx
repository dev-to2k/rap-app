...
function WaitlistForm() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!res.ok) {
      ...
      setError(data.error || "KhÃ´ng lÆ°u ÄÆ°á»£c");
      return;
    }
    setDone(true);
  }
  ...
  if (done) {
    return (
      <div className="mx-auto max-w-md space-y-4 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-6 text-center">
        <h1 className="text-xl font-bold text-white">ÄÃ£ nháº­n early access</h1>
        <p className="text-sm text-zinc-300">Cáº£m Æ¡n báº¡n â mÃ¬nh sáº½ liÃªn há» qua email khi má» thÃªm slot.</p>
        <Link href="/" className="inline-block text-sm text-emerald-400 hover:underline">â Vá» chá»£ beat</Link>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-md space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-zinc-400">
          Chá»£ beat VN-first â lease / WAV+stems / exclusive, checkout VND. KhÃ´ng cáº§n thanh toÃ¡n Äá» vÃ o waitlist.
        </p>
      </div>
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <fieldset className="space-y-2 text-sm">
          <legend className="text-zinc-400">Báº¡n lÃ </legend>
          <div className="flex gap-3">
            {(["producer", "rapper"] as const).map((r) => (
              <label key={r} className={`flex-1 cursor-pointer rounded-xl border px-3 py-2 text-center capitalize ${role === r ? "border-emerald-500 bg-emerald-500/10" : "border-zinc-700"}`}>
        ...
        <button type="submit" disabled={loading}
          ...
          {loading ? "Äang gá»­iâ¦" : "Xin early access"}
        </button>
      </form>
    </div>
  );
